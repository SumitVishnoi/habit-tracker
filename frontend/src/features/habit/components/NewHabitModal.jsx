...
const NewHabitModal = ({ isOpen, onClose, onSubmit, loading }) => {
  ...
  const handleSubmit = (e) => {
    ...
    setError("");
    onSubmit({ name: name.trim(), description: description.trim(), frequency, color });
  };
    ...
    >
      ...
      >
        <div className="flex items-center justify-between mb-6">
          <h2 className="text-xl font-semibold text-[#1F3529]">New habit</h2>
          <button
            type="button"
            onClick={handleClose}
            aria-label="Close"
              ...
              {COLORS.map((c) => (
                <button
                  ...
                  className={`w-8 h-8 rounded-full transition-transform ${
                    color === c ? "ring-2 ring-offset-2 ring-[#2F4A3D] scale-110" : "hover:scale-105"
                  }`}
